...
export default function ReadingPracticePage() {
  ...
  const queryParams = useMemo(() => ({
    ...
    passageNumber: selectedPassage ?? undefined,
    testType: selectedTestType ?? undefined,
    search: searchQuery || undefined,
  }), [page, selectedDifficulty, selectedPassage, selectedTestType, searchQuery]);
  ...
  const handleFilterChange = useCallback((type: 'difficulty' | 'passage' | 'testType', value: DifficultyLevel | number | TestType | null) => {
    if (type === 'difficulty') {
      setSelectedDifficulty(value as DifficultyLevel | null);
    } else if (type === 'passage') {
      setSelectedPassage(value as number | null);
      // Clear full test filter when selecting a specific passage
      if (value !== null) setSelectedTestType(null);
    } else if (type === 'testType') {
      setSelectedTestType(value as TestType | null);
      // Clear passage filter when selecting full test
      if (value === 'FULL_TEST') setSelectedPassage(null);
    }
    setPage(1);
  }, []);
  ...
  const hasActiveFilters = selectedDifficulty || selectedPassage || selectedTestType || searchQuery;
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-neutral-500 dark:text-neutral-400 mt-1">
                Practice with real Cambridge IELTS-style reading passages
              </p>
            </div>
          </div>
      ...
      <div className="bg-white dark:bg-neutral-900 border-b border-neutral-200 dark:border-neutral-800 sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Passage Filter - Only show when not filtering by Full Test */}
            {selectedTestType !== 'FULL_TEST' && availableFilters?.passageNumbers && availableFilters.passageNumbers.length > 0 && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-neutral-500 dark:text-neutral-400">Passage:</span>
                <div className="flex gap-1">
                  {availableFilters.passageNumbers.map((p) => (
                    <button
                      key={p.value}
                      onClick={() => handleFilterChange('passage', selectedPassage === p.value ? null : p.value)}
                      className={cn(
                        'px-3 py-1.5 rounded-lg text-xs font-medium transition-colors',
                        selectedPassage === p.value
                          ? 'bg-primary-600 text-white'
                          : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-700'
                      )}
                    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {!isLoading && !isError && practices.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-neutral-500 dark:text-neutral-400 mb-4">
              {hasActiveFilters ? 'Try adjusting your filters' : 'No reading tests available yet'}
            </p>
            {hasActiveFilters && (
              <button
                onClick={clearFilters}
                        ...
                        <div className="flex items-start gap-4">
                          ...
                          <div className={cn(
                            ...
                            {isFullTest ? (
                              ...
                              <>
                                <span className="text-xs text-neutral-500 dark:text-neutral-400">Passage</span>
                                <span className="text-xl font-bold text-neutral-900 dark:text-white">
                                  {practice.readingPassageNumber || '—'}
                                </span>
                              </>
                            )}
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-6 mt-4">
                              ...
                              <div className="flex items-center gap-2 text-sm text-neutral-500 dark:text-neutral-400">
                                ...
                                <span>{practice.totalQuestions || 13} questions</span>
                              </div>
